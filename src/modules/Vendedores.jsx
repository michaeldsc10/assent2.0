/* ═══════════════════════════════════════════════════
   ...
   Estrutura: users/{uid}/vendedores/{id}
...
/* ─────────────────────────────────────────────────────
   CSS — mesmo padrão visual do módulo Clientes
───────────────────────────────────────────────────── */
const CSS = `
  /* ── Modal ── */
  .modal-overlay {
  ...
  /* grid: ID | Nome | Telefone | Email | Comissão | Observação | Ações */
  .vd-row {
    display: grid;
    grid-template-columns: 72px 1fr 130px 180px 95px 1fr 78px;
...
/**
 * Calcula comissão de uma venda para um vendedor.
 * Base = Lucro = (total - custoTotal).
 * Se lucro < 0, comissão = 0.
 */
...
═══════════════════════════════════════════════════ */
function ModalNovoVendedor({ vendedor, vendedores, onSave, onClose }) {
  ...
  const validar = () => {
    ...
    /* Proibido cadastrar 2 vendedores com nome completamente idêntico */
    if (nomeLimpo) {
      const duplicado = vendedores.some(v =>
        v.nome.trim().toLowerCase() === nomeLimpo.toLowerCase() &&
        v.id !== vendedor?.id
      );
      if (duplicado) e.nome = "Já existe um vendedor com este nome exato.";
    }

    if (form.comissao !== "" && form.comissao !== null) {
  ...
  /* Preview de comissão — exemplo didático */
  const pct = parseFloat(form.comissao);
        ...
        <div className="modal-header">
          <div>
            ...
            <div className="modal-sub">
              {isEdit
                ? `Editando ${vendedor.id} — ${vendedor.nome}`
...
/* ═══════════════════════════════════════════════════
   MODAL: Detalhe do Vendedor (vendas vinculadas)
═══════════════════════════════════════════════════ */
function ModalDetalheVendedor({ vendedor, vendas, onClose }) {
  /* Filtra vendas pelo ID interno do vendedor */
  const vendasVendedor = useMemo(() =>
    vendas.filter(v =>
      v.vendedorId === vendedor.id ||
      v.vendedor?.trim().toLowerCase() === vendedor.nome.trim().toLowerCase()
    ),
    [vendas, vendedor]
  );
  ...
  const totalComissao = vendasVendedor.reduce(
    (s, v) => s + calcComissaoVenda(v, vendedor.comissao),
    0
  );
      ...
      <div className="modal-box modal-box-lg" style={{ maxWidth: 620 }}>
        ...
        <div className="modal-header">
          <div>
            ...
            <div className="modal-sub">
              {[vendedor.id, vendedor.telefone, vendedor.email]
                .filter(Boolean).join(" · ")}
            </div>
          </div>
          <button className="modal-close" onClick={onClose}>
        ...
        <div className="modal-body">
          ...
          <div className="vdh-kpis">
            <div className="vdh-kpi">
              <div className="vdh-kpi-icon"
                style={{ background: "rgba(91,142,240,0.12)" }}>
                <ShoppingBag size={14} color="var(--blue)" />
              </div>
              <div className="vdh-kpi-label">Vendas</div>
              <div className="vdh-kpi-val" style={{ color: "var(--text)" }}>
                {totalVendas}
              </div>
            </div>
            <div className="vdh-kpi">
              ...
              <div className="vdh-kpi-val" style={{ color: "var(--green)" }}>
                {fmtR$(totalFaturado)}
              </div>
            </div>
            <div className="vdh-kpi">
              <div className="vdh-kpi-icon"
                style={{ background: "rgba(200,165,94,0.12)" }}>
          ...
          {vendedor.comissao > 0 && (
            <div className="vdh-commission-box">
              <div>
                <div className="vdh-commission-label">
                  Comissão sobre lucro · {vendedor.comissao}%
                </div>
                <div className="vdh-commission-sub">
                  Calculado sobre (Venda − Custo) de cada venda vinculada
                </div>
              </div>
...
/* ═══════════════════════════════════════════════════
   ...
   Regra: bloqueia se há vendas vinculadas ao vendedor
═══════════════════════════════════════════════════ */
      ...
      <div className="modal-box modal-box-md">
        ...
        <div className="confirm-body">
          ...
          {temVendas ? (
            <>
              <p>
                Não é possível excluir <strong>{vendedor.nome}</strong>.
              </p>
              <div className="confirm-warning">
                Este vendedor possui <strong>{vendasVinculadas} venda(s)</strong> registrada(s).
                Excluir o cadastro quebraria o histórico de vendas.<br /><br />
                Para desativá-lo, edite o registro e remova a comissão ou adicione uma observação indicando o status.
              </div>
            </>
          ) : (
            <p>
...
export default function Vendedores() {
  ...
  useEffect(() => {
    ...
    const vendedoresCol = collection(db, "users", uid, "vendedores");
    ...
    const unsubVendedores = onSnapshot(vendedoresCol, (snap) => {
    ...
    return () => { unsubUser(); unsubVendedores(); unsubVendas(); };
  }, [uid]);
  ...
  const handleAdd = async (form) => {
    ...
    await setDoc(doc(db, "users", uid, "vendedores", newId), {
      ...form,
      criadoEm: new Date().toISOString(),
    });
    await setDoc(
      doc(db, "users", uid),
      { vendedorIdCnt: vendedorIdCnt + 1 },
      { merge: true }
    );
    setModalNovo(false);
  };
  ...
  const handleEdit = async (form) => {
    ...
    await setDoc(doc(db, "users", uid, "vendedores", editando.id), form, { merge: true });
    setEditando(null);
  };

  const handleDelete = async () => {
    if (!uid || !deletando) return;
    await deleteDoc(doc(db, "users", uid, "vendedores", deletando.id));
    setDeletando(null);
  };
  ...
  /* ── Conta vendas por vendedor (para proteção de exclusão) ── */
  const vendasPorVendedor = useMemo(() => {
    const map = {};
    vendas.forEach(v => {
      const vid = v.vendedorId;
      const vnome = v.vendedor?.trim().toLowerCase();
      if (vid) map[vid] = (map[vid] || 0) + 1;
      else if (vnome) {
        const match = vendedores.find(
          vd => vd.nome.trim().toLowerCase() === vnome
        );
            ...
            vendedoresFiltrados.map((v) => {
              ...
              return (
                <div key={v.id} className="vd-row">
                  ...
                  <div>
                    <div className="vd-nome" onClick={() => setDetalhe(v)}>
                      {v.nome}
                    </div>
                    {qtdVendas > 0 && (
                  ...
                  <span className="vd-overflow">{v.observacao || "—"}</span>
