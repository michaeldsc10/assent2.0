...
const normalizarConta = (doc) => {
  const valorTotal    = parseFloat(doc.valorTotal)  || 0;
  const valorPago     = parseFloat(doc.valorPago)   || 0;
  // Se o Firestore já tem valorRestante salvo explicitamente (ex: após pagamento),
  // usa ele; caso contrário recalcula.
  const valorRestante = doc.valorRestante !== undefined && doc.valorRestante !== null
    ? Math.max(parseFloat(doc.valorRestante), 0)
...
const calcStatus = (valorRestante, dataVencimento) => {
  if (parseFloat(valorRestante) <= 0) return "pago";
  const hoje = new Date();
