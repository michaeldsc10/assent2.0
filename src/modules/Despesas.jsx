/* ═══════════════════════════════════════════════════
   ASSENT v2.0 — Despesasnovo.jsx (Versão Final Corrigida)
   Foco: Preservar 100% da interface e funções originais.
   Adição: Apenas lógica de ID Sequencial para exibição.
   ═══════════════════════════════════════════════════ */
...
/* ── HELPERS (Preservados do Despesasnovo.jsx) ── */
...
const proximaData = (dataBase, tipo, intervalo = 1) => { const d = parseDate(dataBase); if (!d) return null; const nova = new Date(d); if (tipo === "mensal")  nova.setMonth(nova.getMonth() + intervalo); if (tipo === "semanal") nova.setDate(nova.getDate() + 7 * intervalo); if (tipo === "anual")   nova.setFullYear(nova.getFullYear() + intervalo); return nova.toISOString().split("T")[0]; };

/* ── LÓGICA DE ID SEQUENCIAL (Adicionada do arquivo Despesas.jsx) ── */
...
export default function Despesas() {
  ...
  useEffect(() => { const unsub = onAuthStateChanged(auth, (user) => setUid(user?.uid || null)); return unsub; }, []);
  ...
  /* ── FUNÇÕES DE AÇÃO (Ajustadas para suportar o campo 'codigo') ── */
  const handleAdd = async (form) => {
    ...
    if (form.parcelado && form.totalParcelas > 1) {
      ...
      for (let i = 0; i < form.totalParcelas; i++) {
        const codigoSeq = gerarIdSequencial(cnt); //
        const dataVenc = new Date(form.vencimento + "T12:00:00");
        dataVenc.setMonth(dataVenc.getMonth() + i);
        const vencStr = dataVenc.toISOString().split("T")[0];

        // Salvando com ID automático (hash) mas incluindo o campo 'codigo' sequencial
        batch.set(doc(collection(db, "users", uid, "despesas")), {
          ...form, codigo: codigoSeq, parcelado: true, parcelaAtual: i + 1,
          vencimento: vencStr, status: calcularStatus(vencStr, "pendente"),
      ...
      await addDoc(collection(db, "users", uid, "despesas"), {
        ...form, codigo: codigoSeq, status: calcularStatus(form.vencimento, "pendente"),
        dataCriacao: new Date().toISOString()
      });
  ...
  const handleEdit = async (form) => {
    ...
    await setDoc(doc(db, "users", uid, "despesas", editando.id), { ...form, status: calcularStatus(form.vencimento, editando.status) }, { merge: true });
    setEditando(null);
  };
  ...
  /* ── FILTROS E MÉTRICAS (Originais do Despesasnovo.jsx) ── */
  ...
  return (
    <>
      ...
      <div className="desp-metrics">
        ...
        <div className="metric-card metric-card-purple"><div className="metric-val metric-val-purple">{fmtR$(metricas.pendente)}</div><div className="metric-label">Pendente Total</div></div>
      ...
      <div className="desp-table-wrap">
        ...
        {despesasFiltradas.map(d => (
          <div key={d.id} className="desp-row">
            {/* EXIBINDO O CAMPO 'codigo' (D0001) EM VEZ DO ID DO DOC */}
            ...
            <span className="cat-badge">{d.categoria || "Fixo"}</span>
      ...
      {/* MODAIS (MANTIDOS ORIGINAIS DO Despesasnovo.jsx) */}
      {modalNovo && <ModalForm onSave={handleAdd} onClose={() => setModalNovo(false)} />}
      {editando && <ModalForm despesa={editando} onSave={handleEdit} onClose={() => setEditando(null)} />}
      {pagando && <ModalPay despesa={pagando} onConfirm={handlePagar} onClose={() => setPagando(null)} />}
      {deletando && <ModalDel despesa={deletando} onConfirm={async () => { await deleteDoc(doc(db, "users", uid, "despesas", deletando.id)); setDeletando(null); }} onClose={() => setDeletando(null)} />}
    </>
  );
}

/* ── COMPONENTES DE MODAL (Estilo original preservado) ── */
function ModalForm({ despesa, onSave, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}><div className="modal-box modal-box-lg"><div className="modal-header"><div className="modal-title">Lançamento</div><button className="modal-close" onClick={onClose}><X size={14}/></button></div><div className="modal-body">
...
function ModalPay({ despesa, onConfirm, onClose }) {
  return (
    <div className="modal-overlay modal-overlay-top" onClick={e => e.target === e.currentTarget && onClose()}><div className="modal-box modal-box-md"><div className="modal-header"><div className="modal-title">Pagar</div><button className="modal-close" onClick={onClose}><X size={14}/></button></div><div className="modal-body"><div className="pay-info"><div style={{display:'flex', justifyContent:'space-between'}}><span>Total:</span><span style={{fontWeight:600}}>{fmtR$(despesa.valor)}</span></div></div></div><div className="modal-footer"><button className="btn-secondary" onClick={onClose}>Cancelar</button><button className="btn-success" onClick={() => onConfirm("pix")}>Confirmar</button></div></div></div>
  );
}

function ModalDel({ despesa, onConfirm, onClose }) {
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}><div className="modal-box modal-box-md"><div className="modal-header"><div className="modal-title">Excluir</div><button className="modal-close" onClick={onClose}><X size={14}/></button></div><div className="modal-body">Excluir <strong>{despesa.descricao}</strong>?</div><div className="modal-footer"><button className="btn-secondary" onClick={onClose}>Cancelar</button><button className="btn-danger" onClick={onConfirm}>Confirmar</button></div></div></div>
  );
}
