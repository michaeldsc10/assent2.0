/* ═══════════════════════════════════════════════════
   ASSENT v2.0 — Despesas.jsx (RESTAURADO + ID SEQUENCIAL)
...
const CSS = `
  ...
  .btn-primary { padding: 9px 20px; border-radius: 9px; background: var(--gold); color: #0a0808; border: none; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; }
  ...
  .btn-danger { padding: 9px 20px; border-radius: 9px; background: var(--red-d); color: var(--red); border: 1px solid rgba(224,82,82,.25); cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 13px; }
  .btn-success { padding: 9px 20px; border-radius: 9px; background: rgba(74,186,130,.12); color: var(--green); border: 1px solid rgba(74,186,130,.25); cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 13px; }
  ...
  .btn-nova-desp { display: flex; align-items: center; gap: 7px; padding: 8px 16px; border-radius: 9px; background: var(--gold); color: #0a0808; border: none; cursor: pointer; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; white-space: nowrap; }
  ...
  .metric-card { border-radius: 12px; padding: 14px 16px; border: 1px solid transparent; position: relative; overflow: hidden; }
  ...
  .filter-chip { padding: 5px 12px; border-radius: 20px; font-size: 11px; font-weight: 500; border: 1px solid var(--border); background: var(--s2); color: var(--text-2); cursor: pointer; }
  .filter-chip.active { background: var(--s3); border-color: var(--border-h); color: var(--text); }
  .filter-select { padding: 5px 10px; border-radius: 8px; font-size: 12px; border: 1px solid var(--border); background: var(--s2); color: var(--text-2); cursor: pointer; outline: none; font-family: 'DM Sans', sans-serif; }

  .desp-table-wrap { margin: 0 22px 22px; background: var(--s1); border: 1px solid var(--border); border-radius: 12px; overflow: hidden; }
  .desp-table-header { padding: 13px 18px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between; }
  .desp-table-title { font-family: 'Sora', sans-serif; font-size: 13px; font-weight: 600; color: var(--text); }
  .count-badge { font-family: 'Sora', sans-serif; font-size: 12px; font-weight: 600; background: var(--s3); border: 1px solid var(--border-h); color: var(--text-2); padding: 2px 10px; border-radius: 20px; }

  .desp-row { display: grid; grid-template-columns: 80px 1fr 100px 110px 110px 90px 100px 110px 90px; padding: 11px 18px; gap: 8px; border-bottom: 1px solid var(--border); align-items: center; font-size: 12px; color: var(--text-2); transition: background .1s; }
  .desp-row-head { background: var(--s2); font-weight: 500; font-size: 10px; text-transform: uppercase; color: var(--text-3); }
  ...
  .cat-manager { background: var(--s2); border: 1px solid var(--border); border-radius: 10px; padding: 12px 14px; margin-top: 10px; }
  .cat-list { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 10px; }
  .cat-item { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: 20px; font-size: 12px; background: var(--s3); border: 1px solid var(--border); color: var(--text-2); }
`;
...
function useCategorias(uid) {
  ...
  useEffect(() => {
    ...
    return onSnapshot(q, (snap) => setCategorias(snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(c => c.ativa !== false)));
  }, [uid]);
  ...
  const desativarCategoria = async (id) => { await setDoc(doc(db, "users", uid, "categorias_despesas", id), { ativa: false }, { merge: true }); };
...
function CategoriasManager({ categorias, onCriar, onDesativar }) {
  ...
  const handleCriar = () => { if (nova.trim()) { onCriar(nova); setNova(""); } };
  return (
    <div className="cat-manager">
      <div className="cat-list">
        {categorias.map(c => <span key={c.id} className="cat-item">{c.nome} <button style={{background:'none', border:'none', color:'var(--red)', cursor:'pointer'}} onClick={() => onDesativar(c.id)}><X size={10}/></button></span>)}
      </div>
      <div style={{display:'flex', gap:6}}>
        <input className="form-input" style={{padding:'5px 10px'}} value={nova} onChange={e => setNova(e.target.value)} placeholder="Nova categoria..." />
        <button className="btn-primary" style={{padding:'5px 10px'}} onClick={handleCriar}>+</button>
      </div>
    </div>
  );
}
...
function StatusBadge({ status }) {
  const map = { pago: { cls: "status-pago", label: "Pago" }, pendente: { cls: "status-pendente", label: "Pendente" }, vencido: { cls: "status-vencido", label: "Vencido" } };
      ...
      <div className="modal-box modal-box-lg">
        ...
        <div className="modal-body">
          ...
          <div className="form-row-3">
            <div><label className="form-label">Valor *</label><input type="number" className="form-input" value={form.valor} onChange={e => setForm({...form, valor: e.target.value})} /></div>
            <div><label className="form-label">Vencimento *</label><input type="date" className="form-input" value={form.vencimento} onChange={e => setForm({...form, vencimento: e.target.value})} /></div>
            <div>
              ...
              <div style={{display:'flex', gap:5}}>
                ...
                <button className="btn-secondary" style={{padding:'0 10px'}} onClick={() => setShowCat(!showCat)}>+</button>
              </div>
            </div>
          </div>
          {showCat && <CategoriasManager categorias={categorias} onCriar={onCriarCategoria} onDesativar={onDesativarCategoria} />}
        </div>
...
export default function Despesas() {
  ...
  const handlePagar = async (d) => {
    await setDoc(doc(db, "users", uid, "despesas", d.id), { status: "pago", dataPagamento: new Date().toISOString().split("T")[0] }, { merge: true });
  };

  const metricas = useMemo(() => {
    const totalPendente = despesas.filter(d => d.status !== "pago").reduce((s, d) => s + (d.valor || 0), 0);
    const totalPago = despesas.filter(d => d.status === "pago").reduce((s, d) => s + (d.valor || 0), 0);
    const vencidas = despesas.filter(d => d.status === "vencido").length;
    return { totalPendente, totalPago, vencidas };
  }, [despesas]);

  if (!uid) return <div className="desp-loading">Carregando...</div>;

  return (
    <>
      <style>{CSS}</style>
      <header className="desp-topbar">
        <div className="desp-topbar-title"><h1>Despesas</h1><p>Controle financeiro</p></div>
      ...
      <div className="desp-metrics">
        <div className="metric-card metric-card-red"><div className="metric-val metric-val-red">{metricas.vencidas}</div><div style={{fontSize:11}}>Vencidas</div></div>
        <div className="metric-card metric-card-purple"><div className="metric-val metric-val-purple">{fmtR$(metricas.totalPendente)}</div><div style={{fontSize:11}}>Pendente</div></div>
        <div className="metric-card metric-card-green"><div className="metric-val metric-val-green">{fmtR$(metricas.totalPago)}</div><div style={{fontSize:11}}>Pago este mês</div></div>
      </div>

      <div className="desp-table-wrap">
        <div className="desp-row desp-row-head">
          <span>ID</span><span>Descrição</span><span>Categoria</span><span>Valor</span><span>Vencimento</span><span>Status</span><span>Pagamento</span><span style={{textAlign:'right'}}>Ações</span>
        </div>
        {despesas.map(d => (
          <div key={d.id} className="desp-row">
            <span className="desp-id">{d.id}</span>
            <span className="desp-desc">{d.descricao}</span>
            ...
            <div style={{display:'flex', gap:4, justifyContent:'flex-end'}}>
              {d.status !== "pago" && <button className="btn-icon btn-icon-pay" onClick={() => handlePagar(d)}><CheckCircle size={13}/></button>}
              <button className="btn-icon btn-icon-edit" onClick={() => setEditando(d)}><Edit2 size={13}/></button>
              <button className="btn-icon btn-icon-del" onClick={async () => await deleteDoc(doc(db, "users", uid, "despesas", d.id))}><Trash2 size={13}/></button>
            </div>
          </div>
        ))}
      </div>

      {modalNovo && <ModalNovaDespesa categorias={categorias} onCriarCategoria={criarCategoria} onDesativarCategoria={desativarCategoria} onSave={handleAdd} onClose={() => setModalNovo(false)} />}
      {editando && <ModalNovaDespesa despesa={editando} categorias={categorias} onCriarCategoria={criarCategoria} onDesativarCategoria={desativarCategoria} onSave={handleEdit} onClose={() => setEditando(null)} />}
    </>
  );
}
