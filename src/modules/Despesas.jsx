...
function useCategorias(uid) {
  ...
  useEffect(() => {
    if (!uid) return;
    const col = collection(db, "categorias_despesas");
  ...
  const criarCategoria = async (nome) => {
    ...
    await addDoc(collection(db, "categorias_despesas"), {
      nome: nomeTrimmed,
      ativa: true,
  ...
  const desativarCategoria = async (id) => {
    await setDoc(doc(db, "categorias_despesas", id), { ativa: false }, { merge: true });
  };
