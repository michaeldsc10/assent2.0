/* ═══════════════════════════════════════════════════
   ASSENT v2.0 — Dashboard.jsx  (refatorado)
   ─────────────────────────────────────────────────
...
/* Mapeia label do menu → chave no Firestore menuVisivel */
const KEY_MAP = {
  "Dashboard":          "dashboard",
  "Clientes":           "clientes",
  "Produtos":           "produtos",
...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .ag-sidebar.collapsed .ag-nav-item.active {
    border-color: rgba(200,165,94,0.2);
    border-left-width: 0;
  }
  ...
  @media (max-width: 720px) {
    ...
    .ag-despesa-grid   { grid-template-columns: repeat(2,1fr); }
...
/** Exibe valor ou skeleton enquanto carrega */
...
export default function Dashboard() {
  ...
  const kpiMain = [
    {
      label:  "Receita Bruta",
      value:  fmtR$(dash.receitaBruta),
      trend:  `${dash.numVendas} venda${dash.numVendas !== 1 ? "s" : ""}`,
      up:     dash.receitaBruta > 0,
      accent: "var(--green)",
      sub:    "no período selecionado",
    },
    {
      label:  "Custo Total",
      value:  fmtR$(dash.custoTotal),
      trend:  dash.custoTotal === 0 ? "—" : `-${fmtR$(dash.custoTotal)}`,
      up:     false,
      accent: "var(--red)",
      sub:    "mercadorias e serviços",
    },
    {
      label:  "Lucro Líquido",
      value:  fmtR$(dash.lucroLiquido),
      trend:  `Margem: ${dash.margem.toFixed(1)}%`,
      up:     dash.lucroLiquido >= 0,
      accent: "var(--gold)",
      sub:    "receita − custo",
    },
  ];
  ...
  const kpiSec = [
    {
      label:  "Ticket Médio",
      value:  fmtR$(dash.ticketMedio),
      accent: "var(--blue)",
      sub:    "por venda no período",
    },
    {
      label:  "A Receber",
      value:  fmtR$(dash.totalAReceber),
      accent: "var(--amber)",
      sub:    "saldo pendente",
    },
    {
      label:  "Projeção 30 dias",
      value:  fmtR$(dash.projecao),
      accent: "var(--purple)",
      sub:    "baseada na tendência",
    },
  ];
      ...
      <div className="ag-content">
              ...
              <PieChart width={130} height={130}>
                ...
                >
                  {(dash.mixData || []).map((_, i) => (
                    <Cell
                      key={i}
                      fill={i === 0 ? "#c8a55e" : "#3ecf8e"}
                      opacity={dash.loading ? 0.2 : (i === 0 ? 0.9 : 0.85)}
                    />
                  ))}
                </Pie>
              </PieChart>
        ...
        <div className="g11">
          ...
          <div className="ag-card-bare">
            ...
            <div className="ag-trow ag-thead" style={{ gridTemplateColumns: "1fr 60px 110px" }}>
            ...
            {dash.loading ? (
              <div className="ag-trow" style={{ gridTemplateColumns: "1fr" }}>
                <span className="ag-skeleton" />
              </div>
            ) : dash.topProdutos.length === 0 ? (
              <div className="ag-empty-row">Nenhuma venda no período</div>
            ) : (
              dash.topProdutos.map((p, i) => (
                <div key={i} className="ag-trow" style={{ gridTemplateColumns: "1fr 60px 110px" }}>
                  <span>{p.nome}</span>
          ...
          <div className="ag-card-bare">
            ...
            <div className="ag-trow ag-thead" style={{ gridTemplateColumns: "1fr 120px" }}>
            ...
            {dash.loading ? (
              ...
              dash.topClientes.map((c, i) => (
                <div key={i} className="ag-trow" style={{ gridTemplateColumns: "1fr 120px" }}>
                  <span>{c.nome}</span>
        ...
        <div className="g1 ag-card-bare">
          ...
          <div className="ag-trow ag-thead" style={{ gridTemplateColumns: "90px 1fr 130px 130px" }}>
          ...
          {dash.loading ? (
            ...
            dash.ultimasVendas.map((v, i) => (
              <div key={v.id || i} className="ag-trow" style={{ gridTemplateColumns: "90px 1fr 130px 130px" }}>
    ...
    <>
      ...
      <div className="ag-app">
        ...
        <header className="ag-global-header">
          ...
          {/* Notificações */}
          <div className="ag-notif" title="Notificações">
            <Bell size={15} />
          </div>
          ...
          <div className="ag-user-area" ref={dropdownRef}>
            ...
            >
              ...
              <ChevronDown
                size={13}
                className={`ag-user-chevron ${dropdownOpen ? "open" : ""}`}
              />
            </div>
